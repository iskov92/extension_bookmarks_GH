...
## Обновления и исправления (Июнь 2023)

- **Исправлена ошибка создания закладок и папок во вложенных папках** - Исправлены функции `showCreateFolderDialog` [764-801] и `showCreateBookmarkDialog` [802-840] в `popup.js`, а также `createStoredBookmark` [59-108] в `src/utils/storage.js` для корректной обработки ID родительских папок.

- **Исправлена ошибка TypeError в классе Navigation** - В файле `src/modules/NavigationModule.js` [10-337] внесены изменения для использования метода `getCurrentParentId()` вместо прямого доступа к свойствам объекта Navigation.

- **Улучшен механизм кеширования для вложенных папок** - В функции `getBookmarksInFolder` [57-121] в `src/utils/bookmarks.js` сокращено время жизни кеша для вложенных папок с 5 до 3 секунд для более быстрого отображения новых элементов.

- **Улучшена обработка ошибок** - В `ErrorHandler.wrapAsync` [69-75] в `src/utils/errorHandler.js` добавлена более детальная обработка ошибок и улучшены сообщения пользователю.

- **Улучшен компонент Modal** - В `src/components/Modal.js` [3-167] добавлена автоматическая валидация ввода, обработка клавиш Escape и Enter, а также улучшен механизм фокусировки на полях ввода.

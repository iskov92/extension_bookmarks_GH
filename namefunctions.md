...
- **Добавлена альтернативная функция получения фавиконов** в `src/utils/storage.js` [820-912] - Реализована новая функция `getFaviconFromFaviconKit` для получения фавиконов через сервис faviconkit.com. Функция принимает URL, извлекает из него домен и создает запрос к API FaviconKit для получения фавикона размером 64px. В качестве резервного варианта (при ошибке или таймауте) используется Google Favicon Service.

- **Улучшена функция ручного обновления фавиконов** в `src/utils/storage.js` [820-912] (функция `updateBookmarkFavicon`) - Функция теперь использует FaviconKit вместо стандартного метода для получения более качественных фавиконов при ручном обновлении. Это обеспечивает альтернативный источник фавиконов при ручном запросе через контекстное меню, когда стандартный метод не смог получить корректный фавикон.

## Обновления и исправления

- **Добавлена функция получения фавиконов через Google** в `src/utils/storage.js` [820-912] - Реализована функция `getGoogleFavicon` для надежного получения фавиконов через сервис Google Favicon Service. Функция использует URL-адрес `https://www.google.com/s2/favicons?domain=example.com&sz=128` для запроса фавикона максимального размера (128px) и обеспечивает корректную обработку ошибок с возвратом стандартной иконки `/assets/icons/link.svg` при неудаче.

- **Улучшена функция ручного обновления фавиконов** в `src/utils/storage.js` [820-912] (функция `updateBookmarkFavicon`) - Функция теперь использует сервис Google Favicon Service для получения более надежных и качественных фавиконов при ручном обновлении через контекстное меню. Это обеспечивает альтернативный способ получения фавиконов для случаев, когда стандартный метод не смог получить корректное изображение.

## Обновления и исправления

- **Улучшено ручное обновление фавиконов** в `src/utils/storage.js` [820-912] (функция `updateBookmarkFavicon`) - Полностью переработана система ручного обновления фавиконов. Теперь при обновлении фавикона через контекстное меню используется многоуровневый подход:

  1. Сначала получение через Google Favicon Service с проверкой на стандартную заглушку Google
  2. При обнаружении заглушки - поиск фавикона непосредственно в HTML-коде страницы с поддержкой различных форматов (`apple-touch-icon`, высокоразрешающие иконки, SVG, обычные favicon.ico)
  3. В случае ошибки - использование стандартной иконки

- **Улучшена детальная проверка фавиконов Google** в `src/utils/storage.js` [820-912] (функция `checkGoogleFavicon`) - Реализован точный алгоритм для определения стандартных заглушек фавиконов Google размером 16x16 и 24x24 пикселей. Алгоритм учитывает несколько параметров:

  1. Размер изображения (16x16 или 24x24 пикселей)
  2. Квадратное соотношение сторон (1:1)
  3. Процент серых пикселей в изображении (>60% для заглушки)
  4. Размер данных изображения (~726 байт для заглушки)

  Функция выводит подробную информацию в консоль для отладки, включая все параметры проверки.

- **Решена проблема с CORS при проверке фавиконов Google** в `src/utils/storage.js` [820-912] (функция `checkGoogleFavicon`) - Оптимизирован алгоритм определения стандартных заглушек Google для работы без доступа к пиксельным данным изображения. Вместо анализа цветов (который может вызывать ошибки CORS), используется надежный метод определения заглушек по размеру:

  1. Изображения размером 16x16 или 24x24 пикселей считаются заглушками
  2. Изображения размером более 32x32 пикселей считаются качественными фавиконами
  3. Изображения других размеров проверяются дополнительно

  Добавлен атрибут `crossOrigin = "anonymous"` для совместимости с образцами изображений с других доменов.

- **Добавлено извлечение фавикона из HTML** в `src/utils/storage.js` [820-912] (функция `getFaviconFromHtml`) - Новая функция, которая получает HTML-код страницы и ищет в нем теги с фавиконами, сортируя их по приоритету качества. Поддерживает apple-touch-icon, различные размеры обычных иконок, SVG и стандартные favicon.ico.

- **Добавлена проверка доступности изображений** в `src/utils/storage.js` [820-912] (функция `checkImageAvailability`) - Вспомогательная функция для проверки доступности фавиконов, найденных в HTML, с таймаутом 1.5 секунды.

- **Добавлен обработчик в background.js** для получения HTML-содержимого страниц при извлечении фавиконов. Обработчик использует `fetch` для получения HTML страниц в обход ограничений CORS.

- **Исправлена обработка относительных путей фавиконов** в `src/utils/storage.js` и `src/components/NestedMenu.js` - Решена проблема с ошибками "Некорректный URL фавикона" при использовании относительных путей:
  1. Добавлен специальный маркер `$EXTENSION_PATH$` для относительных путей внутри расширения
  2. При отображении фавиконов маркер заменяется на абсолютный URL расширения через `chrome.runtime.getURL()`
  3. Улучшена обработка различных типов URL (абсолютные, относительные, с маркером)
  4. Добавлены обходные пути для случаев, когда `getURL()` недоступен

# Информация о функциях расширения GH Bookmarks

## src/utils/storage.js

- **Улучшение функций getFaviconFromHtml, checkGoogleFavicon, updateBookmarkFavicon и getFaviconFast** [914-1606] - Исправлены проблемы с повторным обращением к Google API для фавиконов. Теперь если Google возвращает стандартную заглушку, а из HTML не удалось получить фавикон, всегда используется стандартный логотип расширения /assets/icons/link.svg. Функция getFaviconFast теперь использует стандартный логотип вместо обращения к внешним сервисам, если домен не найден в списке специальных случаев. Добавлена улучшенная проверка качества фавиконов и более надежная обработка ошибок.

## Восстановление фавиконов при работе с корзиной

- **Добавление поддержки сохранения и восстановления фавиконов при работе с корзиной** [2023-06-22] - Включает следующие изменения:
  - `src/utils/storage.js` [80-200] - Функция createStoredBookmark теперь принимает параметр options, который позволяет передать фавикон при создании закладки.
  - `src/utils/bookmarks.js` [55-90] - Функция createBookmark теперь принимает параметр options для настройки закладки, включая фавикон.
  - `src/trash.js` [175-210] - Функция restoreContents теперь передает сохраненный фавикон при восстановлении закладок из папок.
  - `src/trash.js` [210-280] - Функция handleRestore теперь передает фавикон при восстановлении закладки из корзины.
  - `src/popup.js` [540-550] - Добавлено сохранение фавикона при удалении закладки через контекстное меню.
  - `src/modules/ContextMenuModule.js` [317-395] - Добавлено сохранение фавикона при удалении через контекстное меню в модуле.

## Обновления от 09.05.2023

### Исправление массовой установки фавиконов

- **Обновлена функция getFaviconFast** в `src/utils/storage.js` [1376-1596] - Модифицирована для использования DuckDuckGo в качестве основного источника фавиконов при массовом обновлении. Теперь при включении настройки отображения фавиконов сначала пытается получить фавикон через DuckDuckGo, затем через Google, и только если оба источника не сработают, использует стандартную иконку расширения.
- **Экспорт функции getGoogleFavicon** в `src/utils/storage.js` [821-911] - Функция теперь экспортируется для использования в других файлах для получения высококачественных фавиконов для ручного обновления фавиконов через контекстное меню.
- **Оптимизированы функции для работы с фавиконами** - Устранены баги, добавлена лучшая обработка ошибок и более эффективный выбор фавиконов.

## Обновления от 10.05.2023

### Разделение функций получения фавиконов для ручного и массового обновления

- **Удалены упоминания DuckDuckGo из функции getFavicon** в `src/utils/storage.js` [472-473] - Из списка источников фавиконов при ручном обновлении удален сервис DuckDuckGo, оставлен только Google Favicon Service для обеспечения более стабильной работы.
- **Обновлена функция getFaviconFast** в `src/utils/storage.js` [1362-1591] - Для массового обновления фавиконов теперь проверяются стандартные пути вместо использования сервисов DuckDuckGo и Google.
- **Четкое разделение между функциями ручного и массового обновления фавиконов** - Функция `getGoogleFavicon` (используемая в ручном обновлении через контекстное меню) теперь использует только сервис Google, а функция `getFaviconFast` (используемая при массовом обновлении) проверяет локальные пути перед использованием стандартной иконки.

## Обновления и исправления (Новый подход к обновлению фавиконов)

- **Полностью переработана функция updateBookmarkFavicon** [1080-1130] - Реализован новый подход к ручному обновлению фавиконов:
  1. Открывает страницу закладки в новой вкладке
  2. Извлекает фавикон напрямую из DOM страницы
  3. Обновляет фавикон в хранилище
  4. Закрывает вкладку автоматически
- **Добавлена функция extractFaviconFromDOM** [в background.js] - Извлекает фавикон из DOM страницы:
  1. Ищет Apple Touch Icon (обычно высокого качества)
  2. Ищет SVG иконки (векторные, масштабируемые)
  3. Ищет стандартные иконки (rel="icon", rel="shortcut icon")
  4. Проверяет стандартный путь к favicon.ico
  5. Сортирует найденные иконки по размеру и возвращает лучшую
- **Добавлена функция updateBookmarkFaviconInStorage** [в background.js] - Обновляет фавикон в хранилище:
  1. Находит закладку по ID в дереве закладок
  2. Обновляет свойство favicon
  3. Сохраняет обновленное дерево закладок в хранилище

Этот подход обеспечивает получение наиболее качественных фавиконов напрямую с сайта, минуя промежуточные сервисы и проблемы с CORS.

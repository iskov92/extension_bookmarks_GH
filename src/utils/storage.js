 ...
 * Открывает страницу закладки и получает фавикон напрямую из DOM
...
export async function updateBookmarkFavicon(bookmarkId) {
  try {
    ...
    // Отправляем сообщение в background script для обновления фавикона
    return new Promise((resolve) => {
      // Устанавливаем таймаут для случая, если background script не ответит
      const timeoutId = setTimeout(() => {
        resolve({
          success: false,
          error: "Таймаут при обновлении фавикона",
          url: found.item.url,
      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          ...
          // Обновление успешно, возвращаем результат
          resolve({
            success: true,
